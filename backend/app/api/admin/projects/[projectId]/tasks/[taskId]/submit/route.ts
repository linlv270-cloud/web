...
import { submitTask, workflowErrorResponse } from "../../../../../../../../lib/execution-project-workflow";
...
export async function POST(request: Request, { params }: { params: Promise<{ projectId: string; taskId: string }> }) {
  ...
  try {
    return Response.json({ task: submitTask(projectId, taskId, adminPrincipalFromRequest(request)!, await request.json()) });
  } catch (error) {
