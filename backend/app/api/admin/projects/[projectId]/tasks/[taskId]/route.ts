...
import { deleteTask, getTask, updateTask, workflowErrorResponse } from "../../../../../../../lib/execution-project-workflow";
...
export async function GET(request: Request, { params }: { params: Promise<{ projectId: string; taskId: string }> }) {
  ...
  try {
    return Response.json({ task: getTask(projectId, taskId, adminPrincipalFromRequest(request)!) }, { headers: { "cache-control": "no-store" } });
  } catch (error) {
...
export async function PATCH(request: Request, { params }: { params: Promise<{ projectId: string; taskId: string }> }) {
  ...
  if (!projectId || !taskId) return Response.json({ error: "参数无效" }, { status: 400 });
  try {
    return Response.json({ task: updateTask(projectId, taskId, adminPrincipalFromRequest(request)!, await request.json()) });
  } catch (error) {
    return workflowErrorResponse(error);
  }
}
...
export async function DELETE(request: Request, { params }: { params: Promise<{ projectId: string; taskId: string }> }) {
  ...
  if (!projectId || !taskId) return Response.json({ error: "参数无效" }, { status: 400 });
  try {
    return Response.json(deleteTask(projectId, taskId, adminPrincipalFromRequest(request)!));
  } catch (error) {
    return workflowErrorResponse(error);
  }
}
