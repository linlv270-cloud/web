...
import { fileErrorResponse, getProjectFile } from "../../../../../../../lib/execution-project-files";
...
export async function GET(request: Request, { params }: { params: Promise<{ projectId: string; assetId: string }> }) {
  ...
  try {
    return Response.json(getProjectFile(projectId, assetId, adminPrincipalFromRequest(request)!), { headers: { "cache-control": "no-store" } });
  } catch (error) {
