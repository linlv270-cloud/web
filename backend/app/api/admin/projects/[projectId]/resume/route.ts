...
export async function POST(request: Request, { params }: { params: Promise<{ projectId: string }> }) {
  ...
  try { return Response.json(updateProjectLifecycle(projectId, "resume", adminPrincipalFromRequest(request)!, await request.json())); } catch (error) { return projectOperationsErrorResponse(error); }
}
