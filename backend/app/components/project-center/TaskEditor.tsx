...
export function TaskEditor({
  ...
  return <div className={styles.modalBackdrop}><form className={`${styles.modal} ${styles.taskEditor}`} onSubmit={save}>
    <header className={styles.modalHeader}><div><p className={styles.eyebrow}>TASK DEFINITION</p><h2>{initial?.id ? "编辑任务" : "新建任务"}</h2><p>保存后任务保持草稿状态，需通过计划检查后发布。</p></div><button className="icon-button" type="button" onClick={onClose} aria-label="关闭"><X size={18} /></button></header>
    <div className={styles.modalBody}>
      ...
      <section className={styles.formSection}><h3>人员与责任</h3><div className={styles.formGrid}><label className={styles.field}><span>负责人</span><select value={form.ownerId} onChange={(event) => update("ownerId", event.target.value)}><option value="">未指定</option>{activeAccounts.map((account) => <option key={account.id} value={account.id}>{account.name || account.phone}</option>)}</select></label><label className={styles.field}><span>验收人</span><select value={form.approverId} onChange={(event) => update("approverId", event.target.value)}><option value="">未指定</option>{activeAccounts.map((account) => <option key={account.id} value={account.id}>{account.name || account.phone}</option>)}</select></label><label className={styles.field}><span>协作者</span><select multiple value={form.collaboratorIds} onChange={(event) => update("collaboratorIds", [...event.target.selectedOptions].map((option) => option.value))}>{activeAccounts.map((account) => <option key={account.id} value={account.id}>{account.name || account.phone}</option>)}</select></label></div></section>
      ...
      <section className={styles.formSection}><h3>依赖与关键任务</h3><div className={styles.formGrid}><label className={styles.field}><span>依赖任务</span><select multiple value={form.dependencyTaskIds} onChange={(event) => update("dependencyTaskIds", [...event.target.selectedOptions].map((option) => option.value))}>{taskOptions.filter((task) => task.id !== initial?.id).map((task) => <option key={task.id} value={task.id}>{task.title}</option>)}</select></label><label className={styles.checkboxField}><input type="checkbox" checked={form.isCritical} onChange={(event) => update("isCritical", event.target.checked)} /> 关键任务</label></div></section>
