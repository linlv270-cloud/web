...
export function VizImageView({ creators, onSwitchToCard }: Props) {
  ...
  useEffect(() => {
    ...
    const raf1 = requestAnimationFrame(() => {
      const raf2 = requestAnimationFrame(async () => {
        if (cancelled) return;
        // The generation callback intentionally uses the latest selected creators and refs.
  ...
  async function doGenerate() {
    try {
      ...
      if (downloadFormat === "ppt") {
        ...
        fileName = `TDE用户可视化_${dateStr}_${selectedCreators.length}人.pptx`;
        ...
        fileName = `TDE用户可视化_${dateStr}_${selectedCreators.length}人.zip`;
    ...
    return (
      <div style={{ textAlign: "center", padding: "64px 32px" }}>
        ...
        <p style={{ color: C.muted, fontSize: 14, marginBottom: 24, lineHeight: 1.7 }}>
          可视化基于当前筛选结果。请先设置筛选条件，再切换到图片视图。
        </p>
        <button
          type="button"
          onClick={onSwitchToCard}
          style={{ padding: "10px 24px", border: `1px solid ${C.line}`, borderRadius: 6, background: C.surface, cursor: "pointer", fontSize: 14 }}
        >
          返回卡片视图
        </button>
      </div>
    ...
    >
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              <button
                type="button"
                onClick={onSwitchToCard}
                style={{ display: "flex", alignItems: "center", gap: 6, padding: "6px 14px", border: `1px solid ${C.line}`, borderRadius: 6, background: C.surface, cursor: "pointer", fontSize: 13, color: C.text }}
              >
                <LayoutGrid size={16} />
                卡片视图
              </button>
              ...
              <button
                ...
                style={{ display: "flex", alignItems: "center", gap: 6, padding: "6px 14px", border: selectedIds.size === 0 ? `1px solid ${C.line}` : `1px solid ${C.dangerSoft}`, borderRadius: 6, background: selectedIds.size === 0 ? C.surfaceSoft : C.dangerSoft, cursor: selectedIds.size === 0 ? "not-allowed" : "pointer", fontSize: 13, color: selectedIds.size === 0 ? C.faint : C.danger, opacity: selectedIds.size === 0 ? 0.6 : 1 }}
              >
                <Trash2 size={14} />
                清空选择
              </button>
                ...
                >
                  ...
                  >
                    <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0 }}>
                      ...
                      <span
                        style={{
                          fontSize: 14,
                          fontWeight: 600,
                          color: isSelected ? C.onAccent : C.ink,
                          whiteSpace: "nowrap",
                          overflow: "hidden",
                    ...
                    <span
                      style={{
                        fontSize: 12,
                        color: isSelected ? C.darkTextSoft : C.muted,
                        fontWeight: 500,
                        flexShrink: 0,
                        marginLeft: 12,
                      }}
                  ...
                  <div
                    style={{
                      ...
                      marginBottom: `${PAGE_HEIGHT * (1 - previewScale)}px`,
                    }}
                  >
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              <button
                ...
                style={{
                  ...
                  background: page === 1 ? C.surfaceSoft : C.surface,
              ...
              <button
                ...
                style={{
                  ...
                  borderRadius: 6,
                  border: `1px solid ${C.line}`,
                  background: page === totalPages ? C.surfaceSoft : C.surface,
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginRight: 8 }}>
                ...
                <button
                  ...
                  style={{
                    ...
                    borderRadius: 6,
                    background: downloadFormat === "ppt" ? C.accentPressed : C.surface,
                    color: downloadFormat === "ppt" ? C.onAccent : C.text,
                    cursor: "pointer",
                    fontSize: 13,
                ...
                <button
                  ...
                  style={{
                    ...
                    borderRadius: 6,
                    background: downloadFormat === "zip" ? C.accentPressed : C.surface,
                    color: downloadFormat === "zip" ? C.onAccent : C.text,
                    cursor: "pointer",
                    fontSize: 13,
              ...
              <button
                ...
                style={{ padding: "8px 16px", border: `1px solid ${C.line}`, borderRadius: 6, background: C.surface, cursor: "pointer", fontSize: 14, color: C.text, display: "flex", alignItems: "center", gap: 6 }}
              >
                <Download size={16} />
              ...
              <button
                ...
                disabled={selectedIds.size === 0}
                style={{ padding: "8px 16px", border: "none", borderRadius: 6, background: selectedIds.size === 0 ? C.faint : C.accentPressed, color: C.onAccent, cursor: selectedIds.size === 0 ? "not-allowed" : "pointer", fontSize: 14, fontWeight: 600, opacity: selectedIds.size === 0 ? 0.5 : 1, display: "flex", alignItems: "center", gap: 6 }}
              >
                <Download size={16} />
      ...
      {stage === "error" && (
        <div style={{ padding: "32px", flex: 1 }}>
          ...
          <button type="button" onClick={() => setStage("preview")} style={{ padding: "10px 24px", border: "none", borderRadius: 6, background: C.accentPressed, color: C.onAccent, cursor: "pointer", fontSize: 14, fontWeight: 600 }}>
            返回预览
          </button>
        </div>
      )}
